...
const writeLedgerEntry = (value, msg) => {
  ...
  if (cmd === 'register' || verifyCustomer(msg)) {
    ...
    ledger.push({
      value,
      hash: hashToHex(prevHash + stringifiedValue),
...
const balanceHandler = (msg) => {
  ...
  return verified
    ? { balance: balanceReducer(customerId) }
...
const depositHandler = (msg) => {
  ...
  writeLedgerEntry({ cmd: 'deposit', customerId, amount: parseInt(amount) }, msg);

  return balanceHandler(msg);
};
...
const withdrawHandler = (msg) => {
  ...
  if (balance >= amountToWithdraw) {
    writeLedgerEntry({ cmd: 'withdraw', customerId, amount: parseInt(amount) }, msg);
  } else {
